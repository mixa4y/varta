...
def test_fresh_database_reaches_scoped_schema_ceiling(tmp_path: Path) -> None:
    ...
    assert APPLICATION_SCHEMA_CEILING == 6
...
def test_newer_database_is_rejected_before_writable_repository_mode(tmp_path: Path) -> None:
    ...
    connection.execute(
        ...
        VALUES (7, 'system_future', ?, '2026-01-01T00:00:00+00:00')
        """,
        ("f" * 64,),
    )
    connection.close()

    with pytest.raises(NewerSchemaError, match="новішу schema version 7"):
...
def test_concurrent_fresh_startup_serializes_migrations(tmp_path: Path) -> None:
    ...
    connection.close()
    assert history == [(1, 1), (2, 1), (3, 1), (4, 1), (5, 1), (6, 1)]
